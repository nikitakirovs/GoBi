...
def extract_cds_sequences(pyfaidx_landmark, cds_list):
    ...
    for description, location in cds_list:
        ...
        if strand == '+':
            cds_seq = pyfaidx_landmark[start:end].seq
        else:
            cds_seq = pyfaidx_landmark[start:end].reverse.seq

        cds_record = SeqRecord(Seq(cds_seq), id=f'{description}', description=f'({strand})')
...
if __name__ == '__main__':
    landmark = load_landmark('/home/me/gobi/data/apis/apis-mellifera/data/GCF_003254395.2/GCF_003254395.2_Amel_HAv3.1_genomic.fna','NC_037648.1')
    cds_all = parse_gff('/home/me/gobi/data/apis/apis-mellifera/data/GCF_003254395.2/genomic.gff','NC_037648.1')
    ...
    cds_seqs = extract_cds_sequences(landmark,cds_from_range)

    # write cds to file
    output_file = 'apis-mellifera-cds-selection.fasta'
    SeqIO.write(cds_seqs , output_file, 'fasta')
